...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!file || !(file instanceof File)) return json({ error: '请选择图片' }, 400, context);
    if (file.size > 15 * 1024 * 1024) return json({ error: '图片不能超过 15MB' }, 400, context);

    const allowed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
    if (!allowed.includes(file.type)) return json({ error: '仅支持 JPG/PNG/WEBP/GIF' }, 400, context);
    ...
    return json({ error: '上传失败: ' + e.message }, 500, context);
  }
}
